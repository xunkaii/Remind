...
//		num01 = num02; (x) 
		...
		// (byte) 해당 틀로 지정하여 처리
		// 
		// 주의1) 해당 범위 안에 있으면 정상적으로 할당하지만, 해당 범위를 초과하는데 casting하는 경우, 정수의 경우에는 bit 단위로 잘려지기에
		// 원치않는 경과를 확인할 수 있다.
		// 주의2) casting은 반드시 큰 데이터가 작은데이터로 할당하는 경우만 있는 것이 아니라 작은데이터가 큰데이터로 
		//       강제변환에서 연산을 해야하는 경우에도 사용
		// ex) 10/3 ==> 3 / 10/(double)3 ==> 3.333
