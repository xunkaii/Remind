	...
	public static void main(String[] args) {
		...
		/*
 		...
 		4. 연산식(expressions) = 연산자 + 피연산자
 			연산자와 피연자를 이용하여 연산의 과정을 기술한 것
 			ex) 25 + 2, num01 + num02
 			30 * 2 = 5   / 30, 2 : 피연산자 / *, = : 연산자
 			피연산자 + 연산자 : 연산식
		...
		//		나머지 빵의 갯수를 출력하되, 변수를 효과적으로 사용하고 연산자를 처리하여 출력하세요
		int bear = 3;
		int food = 25;
		System.out.println("공평하게 나눈 빵의 갯수: "+(food/bear)+" 나머지: "+(food%bear));
		
		// ex2) 사과가 단가가 2000원 하는 것을 5개 구매하였다. 총비용을 출력하세요
		int apple = 2000;
		System.out.println("총비용: "+(apple*5)+"원");
		
		// ex3) 현재 지갑속에 20000원이 있는데, 교통비로 2000원 2번, 식대로 7000원을 지출하였다. 잔액을 출력하세요 변수활용
		int money = 20000;
		int transPay = 2000;
		int transCnt = 2;
		int foodPay = 7000;
		System.out.println(money+" - "+(transPay*transCnt)+" - "+(foodPay)+" = "+(money-transPay-foodPay));
		/*
 		...
 		/*
			...
			//ex) 총금액과 지출금액을 지정/선언하고, 위에 부호연산자를 이용해서 남은 잔액을 출력하시오
			int cost = 20000;
			int pay = 9000;
			int pay2 = -pay;
			int rest = cost+pay2;
...
/* 			3. 대입연산자 : =
 		...
 		*/
			...
			System.out.println(cnt1++); // 출력하고 더해서 마지막에 cnt 3이 되있는 상황
		...
		/*	
 		6. 비교연산자 : 비교식을 이용해서 true/false값을 리턴하는 연산자를 말한다
 		    ==(동일할 때), !=(동일하지 않을때)
 		    >(초과), <(미만), >=(이상), <=(이하)
 		    int point = 30;
 		    boolean isPass = point==60; // false
 		    boolean isPass2 = point!=60; // true
 		    boolean isPass3 = point>=60; // false
 		    	point가 60이상일 때, true, 현재는 결과 false
 		    	ps) 자바에서만 사용한는 문자열 비교
 		...
 		*/   	
 		...
 		int chNum = 2;
 		boolean isCorrect = chNum == 2; // 정답이 2번일때
